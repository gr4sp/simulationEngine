...
from gr4spModelPathway import getModelTip
import os
import sys
import pandas as pd
import pickle
...
if __name__ == '__main__':
    ...
    def meanvar(x):
        return (1+np.mean(x)) / (1 + np.std(x))

    get_mean_var = functools.partial(meanvar)

    MAXIMIZE = ScalarOutcome.MAXIMIZE
    MINIMIZE = ScalarOutcome.MINIMIZE
    robustnes_functions = [ScalarOutcome(f'{file_prefix} ghg', kind=MINIMIZE,
                                         variable_name='ghg', function=get_mean_var),
                           ScalarOutcome(f'{file_prefix} renew', kind=MAXIMIZE,
                                         variable_name='renew', function=get_mean_var),
                           ScalarOutcome(f'{file_prefix} tariff', kind=MINIMIZE,
                                         variable_name='tariff', function=get_mean_var),
                           ScalarOutcome(f'{file_prefix} wholsesale', kind=MINIMIZE,
                                         variable_name='wholsesale', function=get_mean_var),
                           ScalarOutcome(f'{file_prefix} unmet', kind=MINIMIZE,
                                         variable_name='unmet', function=get_mean_var)]
        ...
        try:
            with MultiprocessingEvaluator(model, n_processes=44) as evaluator:
                results, convergence, optimiser = evaluator.robust_optimize(robustnes_functions, scenarios=scenarios, nfe=nfe_per_round,
                                                                            epsilons=[
                                                                                0.08, 0.01, 0.2, 0.5, 1], restart_optimizer=restart_optimizer,

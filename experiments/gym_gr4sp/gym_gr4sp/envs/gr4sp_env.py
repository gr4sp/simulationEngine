...
class Gr4spEnv(gym.Env):
    #   metadata = {'render.modes': ['human']}

    def __init__(self):
        super(Gr4spEnv, self).__init__()
        # print("OG")
        ...
        self.df = pd.read_csv("scenarios/pathway_nfe_280521.csv")
        self.scenarios_designs = list(self.df.to_records(index=False))[:300]

        # Set database ID according to process ID
        self.db_id = (os.getpid() % 2) + 1

    def handle_broken(self):

        # TODO: HANDLE FINAL YEAR
        # if self.curr_year == 2054:
        # return self.curr_obs, 0.0, True, {}
        ...
        self.reward = self.reward_function(self.last_inds, self.curr_inds)
    ...
    def reset(self):
        try:
            ...
            vals = random.choice(self.scenarios_designs)
            ...
            # self.runGr4sp(experimentId, annualCpi=vals[0], consumption=vals[1],
    ...
    def reward_function(self, before, curr):
        # reward = sum(np.array([self.pct_diff(i, j)
        #                        for i, j in zip(before, curr)]) * self.weights)

        # if reward < -10:
        #     print(self.taken_actions)
        #     print(before, curr)
        return sum(np.array([self.pct_diff(i, j) for i, j in zip(before, curr)]) * self.weights)

    def pct_diff(self, start, end):
        # TODO: How to manage start == 0.0
        return (end - start) / start if start else 0.0

    def get_default_inds(self, year):
        '''
        Return default values for timestep.
        '''
        # TODO: GET 2022
        if year == 2026:
            return np.array([4.63331102, 0.17786988, 4.59453142, 14.6242685, 0.])
        if year == 2030:
            return np.array([4.21423832, 0.20405226, 4.83305597, 17.86625249, 0.])
        if year == 2034:
            return np.array([3.20812981, 0.37676755, 7.40893936, 38.49439872, 0.])
        if year == 2038:
            return np.array([2.74886018, 0.45079118, 12.85221195, 69.90011759, 1.])
        if year == 2042:
            return np.array([2.57777, 0.46717998, 27.49517202, 172.49122279, 52.])
        if year == 2046:
            return np.array([2.52379851, 0.45383358, 43.63000584, 259.83506767, 131.75])
        if year == 2050:
            return np.array([1.45169898, 0.67736449, 38.0756979, 203.64883938, 268.75])
